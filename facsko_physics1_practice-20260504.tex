...
\node[above] at (2.7,1.5) {$F(x)=\left(3+0.5\,x\right)\,\mathrm{N}$};
...
\draw[fill=gray!30] (3.0,-0.5) rectangle (3.8,0.2);
\end{scope}
...
% gravity force
\draw[->, thick] (3.1,1.5) -- (3.1,0.5);
\node[right] at (3.1,1.0) {$mg$};

% friction force
\draw[->, thick,red] (3.25,1.27) -- (3.9,1.7);
\node[above left,red] at (3.8,1.8) {$F_s$};

% normal force
\draw[->, thick,blue] (3.25,1.25) -- (2.7,2.3);
\node[right,blue] at (2.25,2.3) {$N$};
...
\item[(a)] From the graph, the maximum displacement is

$$A=3\,\mathrm{mm}=3\times10^{-3}\,\mathrm{m}.$$

Therefore,

$$\boxed{A=3\,\mathrm{mm}}.$$

\item[(b)] The wave is generated by a resonator operating at

$$f=60\,\mathrm{Hz}=60\,\mathrm{s^{-1}}.$$

Hence,

$$\boxed{f=60\,\mathrm{Hz}}.$$

\pagebreak
\item[(c)] The wavelength is the spatial length of one complete cycle of the wave:

$$\lambda=2\,\mathrm{cm}=0.02\,\mathrm{m}.$$

Hence,

$$\boxed{\lambda=2\,\mathrm{cm}}.$$

\item[(d)] The propagation speed of a wave is

$$v=\lambda f.$$

Substitute the numerical values:

$$v=(0.02\,\mathrm{m})(60\,\mathrm{s^{-1}})=1.2\,\mathrm{m/s}.$$

\pagebreak
Therefore,

$$\boxed{v=1.2\,\mathrm{m/s}}.$$

\item[(e)] The period is the reciprocal of the frequency:

$$T=\frac{1}{f}=\frac{1}{60\,\mathrm{s^{-1}}}=0.0167\,\mathrm{s}.$$

Therefore,

$$\boxed{T=0.0167\,\mathrm{s}}.$$
...
\item A wave propagating along a string can be defined as follows (with x in meters, t in seconds): $y(x,t)=0.02\,\mathrm{m}\sin(30\,\mathrm{s^{-1}}t-4\,\mathrm{m^{-1}}\,x).$ Determine the:
\begin{enumerate}
\item[a)] amplitude
\item[b)] frequency
\item[c)] speed
...
\draw[->] (0,0) -- (7,0) node[right] {$x\,[\mathrm{m}]$};
\draw[->] (0,-1.8) -- (0,1.8) node[above] {$y\,[\mathrm{m}]$};

\draw[blue,thick,samples=200,domain=0:6.3]
plot(\x,{1.2*sin(120*\x)});

\draw[dashed] (0,1.2) -- (6.3,1.2);
\draw[dashed] (0,-1.2) -- (6.3,-1.2);

\node[left] at (0,1.2) {$A=0.02\,\mathrm{m}$};

\draw[<->] (1,-1.45) -- (2.57,-1.45);
\node at (1.8,-1.75) {$\lambda$};
...
$$A=0.02\,\mathrm{m},\qquad \omega=30\,\mathrm{rad/s},\qquad k=4\,\mathrm{rad/m}.$$

\begin{enumerate}
\item[(a)] The amplitude is directly obtained from the equation:

$$\boxed{A=0.02\,\mathrm{m}}.$$

\pagebreak
\item[(b)] The angular frequency and the frequency are related by

$$\omega=2\pi f.$$

Therefore,

$$f=\frac{\omega}{2\pi}=\frac{30}{2\pi}=4.77\,\mathrm{Hz}.$$

Hence,

$$\boxed{f=4.77\,\mathrm{Hz}}.$$

\pagebreak
\item[(c)] The wave speed is

$$v=\frac{\omega}{k}.$$

Substituting the numerical values:

$$v=\frac{30\,\mathrm{rad/s}}{4\,\mathrm{rad/m}}=7.5\,\mathrm{m/s}.$$

Therefore,

$$\boxed{v=7.5\,\mathrm{m/s}}.$$

\pagebreak
\item[(d)] The wavelength is related to the wave number by

$$k=\frac{2\pi}{\lambda}.$$

Therefore,

$$\lambda=\frac{2\pi}{k}=\frac{2\pi}{4}=1.57\,\mathrm{m}.$$

Hence,

$$\boxed{\lambda=1.57\,\mathrm{m}}.$$
...
\draw[->] (0,-0.2) -- (0,2.2) node[above] {$f(x,t)$};

\draw[blue,thick,samples=200,domain=-3.5:3.5]
plot(\x,{1.8*exp(-0.7*(\x-0.8)^2)});

\draw[dashed] (0.8,0) -- (0.8,1.8);
\node[below] at (0.8,0) {$x=vt$};

\draw[->,red,thick] (-2.7,1.5) -- (-1.7,1.5);
\node[red,above] at (-2.2,1.5) {propagation};

\node[blue, right] at (2.2,1.6) {$Ae^{-B(x-vt)^2}$};
\end{tikzpicture}
\end{center}
...
\vskip -10 pt
$$\frac{\partial^{2}f}{\partial x^2}=\left[-2AB+4AB^2(x-vt)^2\right]e^{-B(x-vt)^2}.$$
...
$$\frac{\partial^{2}f}{\partial t^{2}}=v^2\left[-2AB+4AB^2(x-vt)^2\right]e^{-B(x-vt)^2}.$$
...
\draw[blue,thick,samples=300,domain=0:6.3]
plot(\x,{1.2*sin(157*\x)});

\draw[dashed] (0,1.2) -- (6.3,1.2);
\draw[dashed] (0,-1.2) -- (6.3,-1.2);

\node[left] at (0,1.2) {$A=0.06\,\mathrm{m}$};

\draw[<->] (0.5,-1.45) -- (2.0,-1.45);
\node at (1.25,-1.75) {$\lambda=0.08\,\mathrm{m}$};

\draw[dashed] (0.5,0) -- (0.5,-1.45);
\draw[dashed] (2.0,0) -- (2.0,-1.45);
...
$$\boxed{A=0.06\,\mathrm{m}}.$$

From the graph, the length corresponding to $\frac{5}{2}\lambda$ is $20\,\mathrm{cm}$:

$$\frac{5}{2}\lambda=20\,\mathrm{cm}.$$

Therefore,

$$\lambda=\frac{2\cdot20\,\mathrm{cm}}{5}=8\,\mathrm{cm}=\boxed{0.08\,\mathrm{m}}.$$
...
$$f=\frac{v}{\lambda}=\frac{300\,\mathrm{m/s}}{0.08\,\mathrm{m}}=3750\,\mathrm{Hz}.$$

Thus,

$$\boxed{f=3750\,\mathrm{Hz}}.$$
...
\vskip -10 pt
$$\omega=2\pi f=2\pi\cdot3750\,\mathrm{Hz}\approx23600\,\mathrm{rad/s}.$$

\pagebreak
The wave number is

$$k=\frac{2\pi}{\lambda}=\frac{2\pi}{0.08}\approx78.5\,\mathrm{rad/m}.$$

Substituting the parameters gives

$$\boxed{y(x,t)=0.06\,\mathrm{m}\,\sin(23600\,\mathrm{rad\,s^{-1}}\,t-78.5\,\mathrm{rad\,m^{-1}}\,x)}.$$
...
\node[left] at (0,1.2) {$A=2\,\mathrm{mm}$};

\draw[<->] (0.5,-1.45) -- (2.5,-1.45);
\node at (1.5,-1.75) {$\lambda=0.60\,\mathrm{m}$};

\draw[->,red,thick] (4.2,1.25) -- (5.2,1.25);
\node[red,above] at (4.7,1.25) {propagation};
...
\begin{eqnarray*}
  f=30\,\mathrm{Hz}&=&30\,\mathrm{s^{-1}},\\
  \lambda=60\,\mathrm{cm}&=&0.60\,\mathrm{m},\\
  A=2\,\mathrm{mm}&=&2\cdot10^{-3}\,\mathrm{m}.
\end{eqnarray*}
...
$$\omega=2\pi f=2\pi\cdot30=60\pi\,\mathrm{rad/s}.$$

\pagebreak
The wave number is

$$k=\frac{2\pi}{\lambda}=\frac{2\pi}{0.60}=10.47\,\mathrm{rad/m}.$$

Substituting the values into the wave equation gives

$$y(x,t)=2\cdot10^{-3}\,\mathrm{m}\,\sin(60\pi\,\mathrm{s^{-1}}\,t-10.47\,\mathrm{m^{-1}}\,x).$$

Therefore, the wave function in SI units is

$$\boxed{y(x,t)=2\cdot10^{-3}\sin(60\pi\,\mathrm{s^{-1}}\,t-10.47\,\mathrm{m^{-1}}\,x)\,\mathrm{m}}.$$
...
\draw[->] (0,0) -- (7,0) node[right] {$x\,[\mathrm{m}]$};
\draw[->] (0,-1.8) -- (0,1.8) node[above] {$y\,[\mathrm{mm}]$};
...
\node[left] at (0,1.2) {$A=0.27\,\mathrm{mm}$};

\draw[dashed,red] (2,0) -- (2,1.2);
\fill[red] (2,1.2) circle (2pt);
\node[red,below] at (2,0) {$x=0.20\,\mathrm{m}$};

\draw[->,red,thick] (2,1.2) -- (2,1.7);
\node[red,above] at (2,1.55) {transverse motion};

\draw[->,black,thick] (4.4,1.3) -- (5.4,1.3);
\node[above] at (4.9,1.3) {wave propagation};
...
$$A=0.27\,\mathrm{mm}=0.27\cdot10^{-3}\,\mathrm{m}=2.7\cdot10^{-4}\,\mathrm{m}.$$

Thus the wave can be written as

$$y(x,t)=2.7\cdot10^{-4}\sin(12\,\mathrm{s^{-1}}\,x-500\,\mathrm{m^{-1}}\,t)\,\mathrm{m}.$$
...
$$12\,\mathrm{m^{-1}}\,x-500\,\mathrm{s^{-1}}\,t=12\cdot0.20-500\,\mathrm{s^{-1}}\,t=2.4-500\,\mathrm{s^{-1}}\,t.$$

\pagebreak
Therefore,

$$y(0.20\,\mathrm{m},t)=2.7\cdot10^{-4}\sin(2.4-500\,\mathrm{s^{-1}}\,t)\,\mathrm{m}.$$

\begin{enumerate}
\item[(a)] \textbf{Transverse velocity:} The transverse velocity is the time derivative of the displacement:

$$v_y(t)=\frac{\partial y}{\partial t}.$$

Differentiate:

$$v_y(t)=2.7\cdot10^{-4}\cdot(-500\,\mathrm{s^{-1}})\,\mathrm{m}\,\cos(12\,\mathrm{m^{-1}}\,x-500\,\mathrm{s^{-1}}\,t).$$

Hence,
\vskip -5 pt
$$v_y(x,t)=-0.135\cos(12\,\mathrm{m^{-1}}\,x-500\,\mathrm{s^{-1}}\,t)\,\mathrm{m/s}.$$

\pagebreak
At $x=0.20\,\mathrm{m}$:

$$\boxed{v_y(t)=-0.135\cos(2.4-500\,\mathrm{s^{-1}}\,t)\,\mathrm{m/s}}.$$

\item[(b)] \textbf{Acceleration:} The acceleration is the time derivative of the transverse velocity:

$$a_y(t)=\frac{\partial v_y}{\partial t}.$$

Differentiate:

$$\boxed{a_y(t)=-67.5\sin(2.4-500,\mathrm{s^{-1}}\,t)\,\mathrm{m/s^2}}.$$

\item[(c)] \textbf{Displacement at $\mathbf{t=4\,\mathrm{\mathbf{s}}}$:} Substitute $t=4\,\mathrm{s}$ into the displacement function:

$$y(0.20\,\mathrm{m},4\,\mathrm{s})=0.27\sin(12\,\mathrm{m^{-1}}\,\cdot0.20\,\mathrm{m}-500\,\mathrm{s^{-1}}\,\cdot4\,\mathrm{s})\,\mathrm{mm}.$$

This gives

$$y(0.20\,\mathrm{m},4\,\mathrm{s})=0.27\sin(2.4-2000)\,\mathrm{mm}\approx\boxed{0.118\,\mathrm{mm}}.$$
